    ...
    try{
		...
		if($_POST){
			...
			for($i = 1; $i <= $y; $i++){
				...
				// $ses = $db->prepare("SELECT * FROM session WHERE session_id=:session_idd");
				// $arrSes = array(':session_idd'=>$session_idd);
				// $ses->execute($arrSes);
				// $see = $ses->fetch();
				// $session_id = $see['session_name'];

				if($test_score > 30){
					$return = $_POST['return'];
					$_SESSION['error'] = "The Test score must not be Greater than 30 marks";
					$all_purpose->redirect("compute-students-results.php?course_code=$course_code&&session_identification=$session_idd");
				}
				...
				if($confirm ==1){
					...
					$ses = $db->prepare("SELECT * FROM session WHERE session_id=:session_id");
					...
					if($select->rowCount() >0){
						...
						$all_purpose->redirect("student-results.php?course_code=$course_code&&session_identification=$session_idd");
					}else{
