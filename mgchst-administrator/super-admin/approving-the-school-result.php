<?php
	...
	try{
		if(isset($_POST['approve-result'])){
		    ...
		    $detail = $db->prepare("SELECT * FROM student_result WHERE dept_id=:dept_name OR dept_id=:dept_id AND session_id=:session_id AND prog_id=:prog_id AND level_id=:level_id OR level_id=:level_name AND senate_approval=0 ORDER BY course_code ASC");
			$ars = array(':dept_id'=>$dept_id, ':dept_name'=>$dept_name, ':session_id'=>$session_id, ':prog_id'=>$prog_id, ':level_id'=>$level_id, ':level_name'=>$level_name); 
		    $detail->execute($ars); 
		    if($detail->rowCount()==0){
		    	...
		    	<script>
		    		window.location=("approve-result.php?school_name=<?php echo $school_name ?>&&school_identification=<?php echo $school_id ?>");
		    	</script><?php
		    }else{?>
				<ul class="breadcrumb">
				   <ul class="breadcrumb">
				        <li><a href="./">Home</a></li>                    
				        <li><a href="approve-result.php?school_name=<?php echo $school_name ?>&&school_identification=<?php echo  $school_id ?>">Approve Students Results</a></li> 
				        
					    <li><a href="computing-student-results.php">Course Result Panel</a></li> 
					    <li><a href="view-all-students-results.php">View All Results</a></li>    
				        <li class="active">Approving The Students Results For <?php echo  $school_name; ?></li>    
				    </ul> 
				</ul>                     
				<?php
	    	...
	    	$_SESSION['error'] = "Please Fill The Form Below To Approve The Students Results";?>
	    	<script>
	    		window.location=("approve-result.php?school_name=<?php echo $school_name ?>&&school_identification=<?php echo $school_id ?>");
	    	</script><?php
	    }
