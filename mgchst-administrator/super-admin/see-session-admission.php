...
<ul class="breadcrumb">
    <li><a href="./">Home</a></li>  
    <li><a href="see-session-admission.php?academic_session=<?php echo $session_name ?>">See Session Admission List</a></li>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-default">
                 <?php 
                $admit = $db->prepare("SELECT * FROM admission WHERE admission_year=:session_name ORDER BY student_matric_num ASC");
                $arrAdmit = array(':session_name'=>$session_name);
                $admit->execute($arrAdmit);
                if($admit->rowCount()<1){ 
                    $_SESSION['error'] = "No Admission Was Found For $session_name Academic Session" ?>
                     <script>window.location=("session-admission.php");</script>
                    <?php
                }else{ ?>
