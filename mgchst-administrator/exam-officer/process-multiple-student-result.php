		...
		if($_POST){
			...
			for($i = 1; $i <= $y; $i++){
				...
				if($test_score > 30){
					...
					$all_purpose->redirect("compute-multiple-result.php?course_code=$course_code&&session_identification=$session_id&&department=$dept_id");
				}
				...
				if($confirm ==1){
					...
					if($select->rowCount() >0){
						...
						if(!empty($insert->execute($arrIns))){
							...
							$_SESSION['error'] = "Unable to Compute The Students Results for $course_code at the Moment, Please try again later";
							$all_purpose->redirect("compute-multiple-result.php?course_code=$course_code&&session_identification=$session_id&&department=$dept_id");
						}
					}
				}
